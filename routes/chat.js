...
const { chatLimiter, sanitizeInput, validate } = require("../middleware/security");
...
router.get("/", authenticateToken, async (req, res) => {
    try {
        ...
        if (sinceId > 0) {
            ...
            query = "SELECT id, username, message, role, is_read, created_at FROM chat_messages WHERE id > ? ORDER BY created_at ASC LIMIT ?";
            params = [sinceId, limit];
        } else {
            // Ambil pesan terakhir
            query = "SELECT id, username, message, role, is_read, created_at FROM chat_messages ORDER BY created_at DESC LIMIT ?";
            params = [limit];
        }
...
// Send a message
router.post("/", authenticateToken, chatLimiter, sanitizeInput, validate(chatMessageValidation), async (req, res) => {
    try {
